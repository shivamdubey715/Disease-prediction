...
with st.sidebar:
    selected = option_menu(
        ...
        styles={
            ...
            "nav-link-selected": {"background-color": "#a5d8ff"}
        }
    )
...
def validate_input(input_list):
    try:
        return [float(x) for x in input_list if x != ""]
    except ValueError:
        return None
...
if selected == "🏥 Diabetes Prediction":
    ...
    if st.button("Get Diabetes Test Result"):
        ...
        if validated_input and len(validated_input) == 8:
...
if selected == "❤️ Heart Disease Prediction":
    ...
    if st.button("Get Heart Disease Test Result"):
        ...
        if validated_input and len(validated_input) == 13:
